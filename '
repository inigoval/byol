...
from utilities import log_examples, fig2img, embed_dataset, freeze_model, unfreeze_model
    ...
    def on_validation_start(self):
        ...
        for idx, data in enumerate(self.trainer.datamodule.data["val"]):
            if self.config["evaluation"]["linear_eval"]:
                # Initialise linear eval data-set
                lin_eval = Linear_Eval(data["name"], idx)
                # Run setup with encoding forward step in transform via lambda function
                lin_eval.setup(self, data["train"])

                # Add to list of evaluations
                self.val_list.append(lin_eval)

        # if self.config['evaluation']["knn_eval"]:
...
class Linear_Eval(Data_Eval):
    ...
    def setup(self, pl_module, data):
        ...
        # Define encoder, set to eval mode and freeze weights
        # encoder = pl_module.backbone
        # encoder.eval()
        # freeze_model(encoder)

        # Enable gradients to train linear model
        torch.set_grad_enabled(True)

        # Define data-loader with encoder forward step to embed data
    ...
    def step(self, pl_module, X, y):
        ...
        preds = self.model(X)
        self.acc.update(torch.tensor(preds).detach().cpu(), y.detach().cpu())
...
class Linear_Eval_sklearn(Data_Eval):
    """
    Callback to perform linear evaluation at the end of each epoch.
