    ...
    def radio(self):
        augmentations = [
            ...
            A.Lambda(
                ...
                image=BrightnessGradient(limits=(0.0, 1.0)),
                p=self.p,
            ),  # No noise
            A.ElasticTransform(  # Elastically transform the source
                sigma=100, alpha_affine=25, interpolation=1, border_mode=1, value=0, p=self.p
            ),
            A.ShiftScaleRotate(
                shift_limit=0.1,
                scale_limit=0.1,
            ...
            A.Lambda(
                ...
                image=SpectralIndex(mean=-0.8, std=0.2, seed=None),
                p=self.p,
            ),  # Across the whole image
            A.Emboss(
                alpha=(0.2, 0.5), strength=(0.2, 0.5), p=self.p
            ...
            A.Lambda(
                name="Brightness perspective distortion",
                image=BrightnessGradient(limits=(0.0, 1), primary_beam=True, noise=0.01),
                p=self.p,
            ...
            A.CenterCrop(width=85, height=85, p=1),
            A.Lambda(
            ...
            [
                ...
                A.Lambda(
                    ...
                    image=BrightnessGradient(
                        limits=self.limits, primary_beam=False, seed=self.seed, noise=self.noise
                    ),
                    always_apply=True,
                ),
            ]
...
class CustomKernelConvolution:
    ...
    def __call__(self, image, **kwargs):
        ...
        max = 1.0 if np.abs(convolved_image).max() == 0 else np.abs(convolved_image).max()
