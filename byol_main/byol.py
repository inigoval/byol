...
class BYOL_Supervised(BYOL):
    def __init__(self, config):
        ...
        if supervised_head_params["training_mode"] == "classification":
            ...
            self.supervised_head = nn.Sequential(
                BYOLProjectionHead(
                    supervised_in_features, supervised_head_params["hidden"], num_classes
                ),
            ...
            def dirichlet_loss_aggregated_to_scalar(preds, labels, nan_safe=True):
                ...
                if nan_safe:
                    ...
                    elif torch.any(preds <= 0.0):
                        logging.warning("Found concentration predictions below 0 - skipping batch")
                        return torch.zeros_like(preds).sum()  # using "like" for cuda placement

                dirichlet_loss = losses.calculate_multiquestion_loss(
                    labels, preds, supervised_head_params["question_index_groups"]
                )
                ...
                return (
                    empirical_factor
                    * torch.sum(dirichlet_loss)
                    / (num_labelled_galaxies * num_questions)
                )  # over both (batch, question)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if supervised_loss_weight > 1e5:
            ...
            supervised_normalising_constant = (
                torch.abs(contrastive_loss.detach()) / supervised_loss.detach()
            )
            loss = (
                contrastive_loss
                + self.config["supervised_loss_weight"]
                * supervised_normalising_constant
                * supervised_loss
            )
