...
class Lightning_Eval(pl.LightningModule):
    def __init__(self, config):
        ...
        self.knn_acc_test_conf = tm.Accuracy(average="micro", threshold=0)
        self.knn_acc_test_unc = tm.Accuracy(average="micro", threshold=0)
...
class Epoch_Averaged_Test(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if (
            ...
            labels = ["conf", "unc"]
            for label, loader in zip(labels, pl_module.trainer.datamodule.data["test"]):
                loader = DataLoader(
                    pl_module.trainer.datamodule.data["test"],
                    200,
                ...
                for batch in loader:
                    ...
                    pred_labels = knn_predict(
                        ...
                        leave_first_out=pl_module.config["knn"]["leave_first_out"],
                    )
                    ...
                    # Compute accuracy
                    # assert top1.min() >= 0
                    pl_module.knn_acc_test.update(top1, y)

                acc = pl_module.knn_acc_test.compute()
                pl_module.log("test/kNN_acc", pl_module.knn_acc_test.compute())
    ...
    def on_test_epoch_start(self, trainer, pl_module):
        if (
            ...
            pl_module.log("test/kNN_acc_agg", mean(self.test_acc))
