...
class BYOL_Pretext(Lightning_Eval):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred = (self.regress(x0) + self.regress(x1)) / 2
        y_loss = self.regression_loss(y_pred.squeeze(), y.squeeze())

        # coef = self.config["sup_loss_coef"].detach()
        norm = self.config["sup_loss_coef"] * torch.abs(loss).detach() / torch.abs(y_loss).detach()
        y_loss *= norm

        self.log("train/sup_loss", y_loss, on_step=False, on_epoch=True)
